...
class Adam extends Optimizer {
  ...
  override iterate(Objective obj, int t) {
    ...
    for (d : 0 ..< newPoint.nEntries) {
      ...
      v.set(d, beta2 * v.get(d) + (1.0 - beta2) * grad.get(d).pow(2))
      val mHat = m.get(d) / (1.0 - beta1.pow(t+1))
      val vHat = v.get(d) / (1.0 - beta2.pow(t+1))
      newPoint.set(d, oldPoint.get(d) - stepSize * mHat / (vHat.sqrt + epsilon))
    }
